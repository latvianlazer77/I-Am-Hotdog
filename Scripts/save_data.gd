...
# ==========================================
func set_blackout_mode(is_active: bool):
	config.set_value("settings", "blackout", is_active)
	config.save(SAVE_PATH)

func is_blackout_mode() -> bool:
	return config.get_value("settings", "blackout", false)
